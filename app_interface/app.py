...
# Note: Old image_to_url_patch removed - incompatible with Streamlit 1.53+
...
with col1:
    ...
    if uploaded_file is not None:
        ...
        if draw_mode:
            ...
            st.image(image, caption="Original Asset", use_container_width=True)
            ...
            try:
                ...
                if success:
                    ...
                    with col2:
                        ...
                        output_placeholder.image(output_path, caption="🔒 SECURE_OUTPUT_V2.png", use_container_width=True)
...
if st.session_state.history:
    ...
    for i, item in enumerate(reversed(st.session_state.history)):
        with st.expander(f"Run #{len(st.session_state.history)-i} - Bio-Sim: {item['metrics'].get('bio_sim', 'N/A')}", expanded=False):
            ...
            with hc2:
                st.image(item['protected'], caption="Protected", use_container_width=True)
            with hc3:
...
if st.session_state.history:
    ...
    with st.expander("🔬 View AI Segmentation Comparison", expanded=False):
        try:
            ...
            with col_ai1:
                st.image(draw_boxes(img_orig, res_orig), caption=f"BEFORE: AI Detection ({orig_conf:.2f})", use_container_width=True)
            
            with col_ai2:
                st.image(draw_boxes(img_prot, res_prot), caption=f"AFTER: AI Vision ({prot_conf:.2f})", use_container_width=True)
            ...
            with col_ai4:
                ...
                st.image(diff_amp, caption="Perturbation Map (x20)", use_container_width=True)
