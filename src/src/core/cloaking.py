...
class CloakEngine:
    ...
    def apply_defense(self, image_path: str, output_path: str, visual_mode: str = "latent_diffusion", compliance: bool = True, strength: int = 75, face_boost = False, user_mask = None, use_badge: bool = True, badge_text: str = "NO AI TRAIN",
                       ...
                       hidden_command: str = "",
                       max_retries: int = 3) -> tuple[bool, str, dict]:
        """
        ...
        try:
            ...
            mode_str = "FRONTIER" if is_frontier else ("PHANTOM" if is_phantom else "GENERAL")
            ...
            if visual_mode == "latent_diffusion":
                ...
                if self.latent_cloak is None:
                    ...
                    use_lite = is_frontier or is_phantom
                ...
                for attempt in range(max_retries + 1):
                    print(f"--- Adversarial Loop Attempt {attempt+1}/{max_retries+1} (Strength: {current_strength}) ---")
                    config.strength = current_strength
                    config.attempt = attempt # Track retry index for random init in Phase 15.5
                    ...
                    if is_frontier:
                        ...
                        # SKIP badge/text for Frontier Lite (clean geometric output)
                    elif is_phantom:
                        print(f"[CloakEngine] PHASE 15.5: Using Phantom Pixel Boost (Attempt {attempt})...")
                        img = self.latent_cloak.protect_phantom(image_path, config=config, user_mask=user_mask)
                    else:
